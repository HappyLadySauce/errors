...
// Format 实现 fmt.Formatter 接口。https://golang.org/pkg/fmt/#hdr-Printing
//
// 动词：
//     %s  - 返回用户安全的错误字符串映射到错误代码或
//       ┊  如果没有指定，则返回错误消息。
//     %v     别名 %s
// 标志：
//      #      JSON 格式化输出，用于日志记录
...
//      +     输出完整错误堆栈详细信息，用于调试
// 示例：
//      %s:   错误 for internal read B
//      %v:   错误 for internal read B
//      %-v:  错误 for internal read B - #0 [/home/lk/workspace/golang/src/github.com/marmotedu/iam/main.go:12 (main.main)] (#100102) Internal Server Error
//      %+v:  错误 for internal read B - #0 [/home/lk/workspace/golang/src/github.com/marmotedu/iam/main.go:12 (main.main)] (#100102) Internal Server Error; error for internal read A - #1 [/home/lk/workspace/golang/src/github.com/marmotedu/iam/main.go:35 (main.newErrorB)] (#100104) Validation failed
//      %#v:  [{"error":"error for internal read B"}]
//      %#-v:  [{"caller":"#0 /home/lk/workspace/golang/src/github.com/marmotedu/iam/main.go:12 (main.main)","error":"error for internal read B","message":"(#100102) Internal Server Error"}]
//      %#+v:  [{"caller":"#0 /home/lk/workspace/golang/src/github.com/marmotedu/iam/main.go:12 (main.main)","error":"error for internal read B","message":"(#100102) Internal Server Error"},{"caller":"#1 /home/lk/workspace/golang/src/github.com/marmotedu/iam/main.go:35 (main.newErrorB)","error":"error for internal read A","message":"(#100104) Validation failed"}]
...
// finfo 是格式化信息。
// sep 是分隔符。
// flagDetail 是是否输出详细信息。
// flagTrace 是是否输出堆栈跟踪。
// modeJSON 是是否输出 JSON 数据。
// 返回 JSON 数据和字符串缓冲区。
...
// buildFormatInfo builds the format information for the error.
// buildFormatInfo 构建错误的信息。
